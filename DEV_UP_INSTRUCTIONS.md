...
**Company lens (independent):** Runpod (`runpod`)  
**Innovation:** Pod Preempt Receipt  
**Scaffold batch:** 2026-08-10T0924Z

## Mission

Implement a **real, testable** central mechanism that addresses the bottleneck below. Do **not** claim Runpod affiliation, proprietary access, or production deployment.
...
Emit preempt receipts with checkpoint hints and restart grants; refuse silent disappearance.

## Hard rules (fail closed)

1. **No affiliation theater** — never state or imply Runpod employment, endorsement, or proprietary systems access.
...
File: `src/pod_preempt_receipt.py`

- Expand `PodPreemptReceipt` into a complete, self-contained implementation.
- Public API must stay stable enough that tests in `tests/test_pod_preempt_receipt.py` can be upgraded (not gutted).
- Include at least:
...
### 3. Strengthen tests
Files: `tests/test_pod_preempt_receipt.py`, `tests/test_adversarial.py`
...
- Cloud deploy, customer pilots, proprietary Runpod APIs
